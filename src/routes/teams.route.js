...
router.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      if (!myInventory){
        return res.status(404).json({message:"해당 플레이어를 보유하고 있지 않습니다"});
      }
      ...
      if (existingPlayerCount >= 3) {
        return res.status(400).json({ message: "팀에 추가할 수 있는 플레이어 수는 최대 3명입니다." });
      }
...
router.get(
  "/teams/:userId", async (req, res, next) => {
